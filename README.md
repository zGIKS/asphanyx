...
On startup, SQLx migrations from `migrations/` are executed automatically (including `provisioned_databases` table creation).
Provisioning audit events are stored centrally in `provisioning_audit_events` (admin database), not in tenant databases.
